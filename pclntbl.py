...
class Pclntbl():
    ...
    def parse_hdr(self):
        ...
        if self.magic_number == common.MAGIC_116:
            ...
            idc.set_cmt(self.start_addr + 8 + 2 * self.ptr_sz,
                        f"func names table offset, real addr: {self.funcnametab_addr:#x}", 0)
            idaapi.auto_wait()
            ...
            idc.set_cmt(self.start_addr + 8 + 4 * self.ptr_sz,
                        f"src file table offset, real addr: {self.srcfile_tbl_addr:#x}", 0)
            ...
            idc.set_cmt(self.start_addr + 8 + 6 * self.ptr_sz, f"func table offset, real addr: {self.func_tbl_addr:#x}",
                        0)
            idc.set_name(self.func_tbl_addr, "pc0", flags=idaapi.SN_FORCE)
            idaapi.auto_wait()
            ...
            self.funcnametab_addr = self.start_addr + funcname_tbl_off
            idc.set_cmt(self.start_addr + 8 + 3 * self.ptr_sz,
                        f"func names table offset, real addr: {self.funcnametab_addr:#x}", 0)
            idaapi.auto_wait()
            ...
            self.srcfile_tbl_addr = self.start_addr + srcfile_tbl_off
            idc.set_cmt(self.start_addr + 8 + 5 * self.ptr_sz,
                        f"src file table offset, real addr: {self.srcfile_tbl_addr:#x}", 0)
            idc.set_name(self.srcfile_tbl_addr, "runtime_filetab", flags=idaapi.SN_FORCE)
            idaapi.auto_wait()
            ...
            self.func_tbl_addr = self.start_addr + funtbl_off
            idc.set_cmt(self.start_addr + 8 + 7 * self.ptr_sz, f"func table offset, real addr: {self.func_tbl_addr:#x}",
                        0)
    ...
    def parse_funcs_118(self):
        ...
        for func_idx in range(self.func_num):
            ...
            common._info(hex(self.func_tbl_addr))
            func_entry_off = common.read_mem(curr_functab_addr, forced_addr_sz=4)
            common._info(hex(curr_functab_addr + 4))
    ...
    def parse_srcfile_112(self):
        ...
        common._info("--------------------------------------------------------------------------------------")
        common._info(
            f"Source File paths(Total number: {self.srcfile_num}, default print results are user-defind files):\n")
        for idx in range(self.srcfile_num):
            ...
            if srcfile_path is None or len(srcfile_path) == 0:
                common._error(
                    f"Failed to parse the [{idx + 1}] src file(off: {srcfile_off:#x}, addr: @ {srcfile_addr:#x})")
                continue
    ...
    def parse_srcfile_latest(self):
        ...
        common._info("--------------------------------------------------------------------------------------")
        common._info(
            f"Source File paths(Total number: {self.srcfile_num:#x}, default print results are user-defind files):\n")
...
class FuncStruct():
    ...
    def parse(self, is_test=False):
        ...
        if raw_name_str and len(raw_name_str) > 0:
            ...
            common._debug(
                f"Failed to get valid function name for func st @ {self.addr:#x}, target func name addr: {name_addr:#x}")
        ...
        if raw_name_str and len(raw_name_str) > 0:
            idc.set_cmt(self.addr + self.pclntbl.ptr_sz,
                        f"Func name offset(Addr @ {name_addr:#x}), name string: {raw_name_str.decode('UTF-8', errors='ignore')}",
                        0)
        else:
        ...
        if self.pclntbl.magic_number == common.MAGIC_112:
            ...
            self.nfuncdata = common.read_mem(self.addr + self.pclntbl.ptr_sz + 6 * 4, forced_addr_sz=4,
                                             read_only=is_test)
            self.npcdata = common.read_mem(self.addr + self.pclntbl.ptr_sz + 7 * 4, forced_addr_sz=4, read_only=is_test)
        elif self.pclntbl.magic_number == common.MAGIC_116:
            self.args = common.read_mem(self.addr + self.pclntbl.ptr_sz + 4, forced_addr_sz=4, read_only=is_test)
            self.deferreturn = common.read_mem(self.addr + self.pclntbl.ptr_sz + 2 * 4, forced_addr_sz=4,
                                               read_only=is_test)
            self.pcsp = common.read_mem(self.addr + self.pclntbl.ptr_sz + 3 * 4, forced_addr_sz=4, read_only=is_test)
            self.pcfile = common.read_mem(self.addr + self.pclntbl.ptr_sz + 4 * 4, forced_addr_sz=4, read_only=is_test)
            self.pcln = common.read_mem(self.addr + self.pclntbl.ptr_sz + 5 * 4, forced_addr_sz=4, read_only=is_test)
            self.npcdata = common.read_mem(self.addr + self.pclntbl.ptr_sz + 6 * 4, forced_addr_sz=4, read_only=is_test)
            self.cuOffset = common.read_mem(self.addr + self.pclntbl.ptr_sz + 7 * 4, forced_addr_sz=4,
                                            read_only=is_test)
            ...
            try:
                ...
                common._debug(
                    f"Invalid funcID. Curr func entry: {real_func_addr.start_ea:#x}, func st addr: {self.addr:#x}, func id: {self.func_id:#x}")
