...
def test_firstmoduledata(possible_addr):
    ...
    offset = read_mem(mod_data.pcheader_addr + 8 + 6 * ADDR_SZ, read_only=True)
    if (mod_data.pcheader_addr + offset) == mod_data.pclntbl_addr:
...
def find_first_moduledata_addr():
    ...
    if not is_stripped(): # not stripped, find firstmoduledata by symbol name
        ...
        while curr_addr <= idc.BADADDR:
            if idc.get_wide_dword(read_mem(curr_addr, read_only=True)) & 0xFFFFFFFF == magic_num: # possible firstmoduledata
...
class ModuleData():
    ...
    def parse(self, is_test=False):
        ...
        self.funcnametab_num = read_mem(self.start_addr+2*ADDR_SZ, read_only=is_test)
        ...
        self.cutab_num = read_mem(self.start_addr+5*ADDR_SZ, read_only=is_test)
        ...
        self.srcfile_num = read_mem(self.start_addr+8*ADDR_SZ, read_only=is_test)
        ...
        self.pctab_num = read_mem(self.start_addr+11*ADDR_SZ, read_only=is_test)
        ...
        self.func_num = read_mem(self.start_addr+18*ADDR_SZ, read_only=is_test)
        ...
        self.type_num = read_mem(self.start_addr+41*ADDR_SZ, read_only=is_test)
        ...
        self.itab_num = read_mem(self.start_addr+44*ADDR_SZ, read_only=is_test)
        ...
        self.ptab_num = read_mem(self.start_addr+47*ADDR_SZ, read_only=is_test)
        ...
        if not is_test:
            ...
            idc.set_cmt(self.start_addr + 5*ADDR_SZ, "cutab size", 0)
            idc.set_cmt(self.start_addr + 6*ADDR_SZ, "cutab capacity", 0)
            idc.set_cmt(self.start_addr + 7*ADDR_SZ, "source files table addr",0)
            idc.set_cmt(self.start_addr + 8*ADDR_SZ, "source files number",0)
            idc.set_cmt(self.start_addr + 9*ADDR_SZ, "source files table capacity",0)
            idc.set_cmt(self.start_addr + 10*ADDR_SZ, "pc addr", 0)
            idc.set_cmt(self.start_addr + 11*ADDR_SZ, "pc size", 0)
            idc.set_cmt(self.start_addr + 12*ADDR_SZ, "pc capacity", 0)
            ...
            idc.set_cmt(self.start_addr + 17*ADDR_SZ, "funcs number",0)
            ...
            idc.set_cmt(self.start_addr + 37*ADDR_SZ, "test section map addr",0)
            idc.set_cmt(self.start_addr + 38*ADDR_SZ, "test section map length",0)
            idc.set_cmt(self.start_addr + 39*ADDR_SZ, "test section map capacity",0)
            idc.set_cmt(self.start_addr + 40*ADDR_SZ, "typelink addr",0)
            idc.set_cmt(self.start_addr + 41*ADDR_SZ, "types number",0)
            idc.set_cmt(self.start_addr + 42*ADDR_SZ, "types table capacity",0)
            idc.set_cmt(self.start_addr + 43*ADDR_SZ, "itabslink addr",0)
            idc.set_cmt(self.start_addr + 44*ADDR_SZ, "itabs number",0)
            idc.set_cmt(self.start_addr + 45*ADDR_SZ, "itabs caapacity",0)
            idc.set_cmt(self.start_addr + 46*ADDR_SZ, "ptab addr",0)
            idc.set_cmt(self.start_addr + 47*ADDR_SZ, "ptab num",0)
